...
void Reassembler::inserthelp(uint64_t first_index,std::string data){
  ...
  if(lb!=ub){//ub之前的和lb前面的不是同一块,那样需要考虑是否要将这部分拼进去
    if(std::prev(ub)->first+std::prev(ub)->second.size()>(first_index+data.size())){
      data.append(std::move(std::prev(ub)->second.substr((first_index+data.size()-std::prev(ub)->first))));//
    }
  }
  //考虑将data拼进lb前面的
...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  const auto checkclose=[&](){
    if(haslastSubstr&&unassembled.empty()){//如果已经没有被assemble的数据报，并且我知道已经有了最后一个子串，那么就要关闭我们的写端了（或者说，in_bound)
  ...
  if(acknum==first_index){//来的正是我需要的
    ...
    if(it==unassembled.end()){//说明所有在unassembled中的元素的索引都比acknum小，清空unassembled这个容器再返回即可。
    ...
    inserthelp(first_index,std::move(data));
  }
