...
{
  uint64_t ret{static_cast<uint64_t>(raw_value_-zero_point.raw_value_)};//这里首先得到32位的绝对地址，然后让它和checkpoint的高32位组合，来得到一个待定的返回值
    ret=ret|(checkpoint>>32<<32);
    // debug("checkpoint = {} ret = {}\n", checkpoint,ret);
    if(ret>checkpoint&&(ret>=1ULL<<32)){//如果这时候retbicheckpoint要大，那么它是有可能离checkpoint更近的
